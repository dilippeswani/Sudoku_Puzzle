...
def check_associated_small_squares_for_a_valid_value(sudoku_puzzle):
    for index, values in dict_index_value_zero_r_c_ss.items():
        if sudoku_puzzle[index]:
            continue
        else:
            list_of_values_from_small_square_for_this_index = \
                get_all_values_of_associted_small_squares_for_this_index(index)

    result, status = check_for_max_occurance_four_in_values(list_of_values_from_small_square_for_this_index, index)
    if status:
        update_sudoku_puzzle(index, result)


def update_sudoku_puzzle(index, result):
    assert sudoku_puzzle[index] == 0
    sudoku_puzzle[index] = result


def check_for_max_occurance_four_in_values(check_for_max_occurance_four_in_values, index):
    values = dict_possible_value.get(index)
    for value in values:
        repetition_count_of_value = check_for_max_occurance_four_in_values.count(value)
        if repetition_count_of_value == MAX_REPEATATION_OF_A_VALUE:
            return value, True
    return None, False


def get_all_values_of_associted_small_squares_for_this_index(index):
    list_of_common_small_square_keys = []
    for keys, values in c_small_squares.items():
        if index in values:
            list_of_common_small_square_keys.append(keys)

    list_of_common_indexes_from_small_square_keys = ((c_small_squares.get(list_of_common_small_square_keys[0])) +
                                                     (c_small_squares.get(list_of_common_small_square_keys[1])))
    list_of_values_from_small_square_indexes = []
    for val in list_of_common_indexes_from_small_square_keys:
        if sudoku_puzzle[val] != 0:
            list_of_values_from_small_square_indexes.append(sudoku_puzzle[val])
    return list_of_values_from_small_square_indexes

def compare_ss(sudoku_puzzle):
    for index, value in enumerate(sudoku_puzzle): # Can we not use the "zero values" dict to check for values
        if value == 0:
            list_of_c_ss_keys = []
            for keys, values in c_small_squares.items(): # does the c signify column
                if index in values:
                    list_of_c_ss_keys.append(keys)
                    print("list_of_c_ss_keys", list_of_c_ss_keys, "\n")
            common_list_c_ss_index = (
                    (c_small_squares.get(list_of_c_ss_keys[0])) + (c_small_squares.get(list_of_c_ss_keys[1]))) # What is the purpose of this hard coding of indices
            # Are we trying to check for two adjacent empty values?
            common_list_c_ss_value = []
            print("common_list_c_ss_index", common_list_c_ss_index)
            for val in common_list_c_ss_index:
                if sudoku_puzzle[val] != 0:
                    common_list_c_ss_value.append(sudoku_puzzle[val])
                    print("common_list_c_ss_value", common_list_c_ss_value)
                    for key_s, value_s in dict_possible_value.items():
                        if key_s == index:
                            possible_value_for_index = dict_possible_value.get(key_s)
                            print("possible_value_for_index", possible_value_for_index)
